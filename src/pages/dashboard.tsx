...
import { getVacationRequests } from 'apiSdk/vacation-requests';
import { VacationRequestInterface } from 'interfaces/vacation-request';
import { getEmployees } from 'apiSdk/employees';
import { EmployeeInterface } from 'interfaces/employee';

import { VacationRequestListPage as ShowCaseList } from 'pages/vacation-requests';

function DashboardPage() {
  const { session } = useSession();

  const vacationRequestStat = useSWR<PaginatedInterface<VacationRequestInterface>>(
    () => `/vacation-requests`,
    () => getVacationRequests({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      ...
      result: vacationRequestStat?.data?.totalCount,
      isLoading: vacationRequestStat.isLoading,
    },
    {
      title: 'Employees',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`vacation-requests`} passHref legacyBehavior>
                  <Button
